...
- 实现类型系统与 Schema/Value 序列化：让 Tuple 从裸字节升级为按 Schema 编解码的结构化数据，补齐 Stage 3 的基础能力并加测试覆盖。

  - 建立 Catalog 与元数据持久化：记录表名、Schema、firstPageId（系统表或元数据页），让 TableHeap 可从 Catalog 恢复而不是手动传入页面 ID。
  - 完善空间管理：支持删除后的 slot 复用/压缩（或 free list），并补上 DiskManager 的空闲页回收，打通 BufferPoolManager.deletePage 的 deallocate。
  - 引入最小执行层：先做 SeqScan（基于 TableIterator）+ 简单过滤/投影接口，为后续执行器树做铺垫。
